  ...
  <div class="contain-dash flex">
    <div class="sidebar flex column between">
      ...
      <div class="sidebar-buttons flex column l5xl">
        <button type="button" class="t2xl" @click="routerTo('Projects')" :class="{ current: isCurrent('Projects') }">
          <h2>Meus Projetos</h2>
        </button>
        <button type="button" class="t2xl" @click="routerTo('Users')" :class="{ current: isCurrent('Users') }">
          <h2 v-if="isAdm">Usuários</h2>
        </button>
        <button type="button" class="t2xl" @click="routerTo('Config')" :class="{ current: isCurrent('Config') }">
          <h2>Configurações</h2>
        </button>
      </div>
